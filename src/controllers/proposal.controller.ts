...
export class ProposalController {
  static async create(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      const validationResponse = check({
        userId,
        areaId,
        categoryId,
        title,
      ...
      if (peopleInChargeId) {
        const peopleInCharegeExist = await UserService.getOneById(
          peopleInChargeId
        );
      ...
      if (customCategory) {
        const categoryByName = await CategoryService.getOneByName(
          customCategory
        );
      ...
      await ProposalStatusService.create({
        proposalId: data.id,
        userId,
        status: "baru",
      });
      ...
      const updatedData = await ProposalService.update(data.id, {
  ...
  static async getAll(req: Request, res: Response) {
    const { search, page, limit, areaId, status, categoryId } = req.query as {
      search?: string;
      page?: number;
      limit?: number;
      areaId?: string;
  ...
  static async delete(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      const userPermission = await RolePermissionService.getAllByRoleId(
        roleId as string
      );
      ...
      const isOwner = proposalExist.userId === userId;
      const hasDeletePermission = userPermission.some(
        (permission) => permission.permission.name === "Hapus Proposal"
      );
      ...
      if (isOwner && proposalExist.status !== "baru") {
        return res.status(400).json({
          status: "error",
          message: "Proposal sudah diproses",
        });
  ...
  static async export(req: Request, res: Response) {
    const { areaId, startDate, endDate } = req.query as {
      areaId?: string;
      startDate?: string;
      endDate?: string;
    };
    ...
    try {
      const proposals = await ProposalService.getExportData(
        parsedStartDate,
        parsedEndDate,
        areaId
      );
